...
check "cache dir exists and is writable by the remote user" bash -c 'test -d /var/cache/graphify && test -w /var/cache/graphify'
check "cache dir is owned by the remote user" bash -c '[ "$(stat -c %U /var/cache/graphify)" = "$(id -un)" ]'
