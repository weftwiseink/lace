...
- `claude migrate-installer` moves an existing npm-global install to the native user-local install. (Author-cited: this specific command was not on the setup docs page consulted, though that page confirms essentially every other native-install claim here verbatim. The preferred design path is the direct native install, which does not need this migrate step.)
...
- Launcher outside the volume. The native launcher at `~/.local/bin/claude` is a symlink into `~/.local/share/claude/versions/`. If only `~/.local/share/claude` is on the volume and `~/.local/bin` is not, a rebuild resets the launcher symlink from the (baseline) image while the volume holds newer versions. A named volume mounts exactly one target path, so this is not a free "either/or"; the candidate mechanisms and their costs must be weighed explicitly:
  - Persist only `~/.local/share/claude`, and let the installer-managed `~/.local/bin/claude` symlink self-repoint on the first post-rebuild auto-update. Cost: one baseline-version startup after each rebuild until the background updater runs. The docs confirm the updater leaves a custom launcher in place and installs versions under `versions/` regardless (behavior as of v2.1.207; earlier versions overwrote a custom launcher), so this is viable.
  - Mount a second volume over `~/.local/bin`. Cost: that mount shadows all other contents of `~/.local/bin` (only image-time contents survive via first-population, and nothing repopulates them on later rebuilds), so any other tool that expects to install there is affected. This coupling is the least attractive option.
  - Maintain a volume-internal `current` symlink under `~/.local/share/claude` that a stable, build-time-baked launcher dereferences. Cost: a small amount of custom launcher wiring, but only one volume is needed and the launcher never resets.
  The choice (a single-volume self-repointing launcher versus a volume-internal `current` symlink; avoid the second-volume shadowing option) is a scratch-container decision deferred to implementation, but the enumeration is settled here.
...
> NOTE(opus/claude-volume-native-update): This is a design and decision proposal, not a build-out. Per the propose-revise-only scope, no implementation follows immediately. The reproducibility stance (condition 3) and the supersession-after-verification bookkeeping are policy calls the review loop and overseer should confirm.
...
3. lace passthrough check: confirm the CLI, driving podman on the `--buildkit never` path, honors the feature-native named-volume mount (the go/no-go gate), and that it composes with lace's existing bind mounts and injected lifecycle commands.
...
- CLI+podman honoring of a feature-native named-volume `mounts` entry on lace's `devcontainer up --buildkit never` path: expected but unverified from this worktree. The precise question is whether the devcontainer CLI, driving podman on the legacy-builder path, creates and attaches the named volume from a feature-declared top-level `mounts` entry (with `${devcontainerId}` substitution). This must be tested against podman specifically, not merely docker. This is the load-bearing go/no-go condition (condition 2) and the direct analogue of the npm design's unverified `postCreateCommand` assumption.
- Launcher/volume scope decision: which of the enumerated launcher mechanisms (single volume with a self-repointing installer launcher; single volume with a volume-internal `current` symlink and a stable baked launcher; avoid the second-volume-over-`~/.local/bin` shadowing option) to adopt. The docs describe the updater leaving a custom launcher in place (as of v2.1.207), but the exact interaction with a volume-backed `versions/` tree across recreation needs a scratch-container check.
